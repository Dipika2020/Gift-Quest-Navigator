...
class GameManager:
    ...
    def ucs_search(self) -> list[State]:
        frontier = MinHeap()  # Use MinHeap or a priority queue for the frontier
        frontier.add(Node(self.init_state, 0))  # Add initial state with cost 0
        ...
        while not frontier.is_empty():
            ...
            if State.is_goal(current_node.state, self.map.points):
                return self.extract_path_list(current_node)

            explored.add(current_node.state)

            actions = State.successor(current_node.state, self.map)
            for action in actions:
                # Apply the action to the current state to get the next state
                next_state = current_node.state.apply_action(action)
                # Calculate the cost for the next state
                next_cost = current_node.path_cost + self.compute_cost(current_node.state, action)

                if next_state not in explored and not frontier.__contains__(next_state):
                    frontier.add(Node(next_state, next_cost))

                elif frontier.__contains__(next_state) and next_cost < frontier.value_of(next_state).path_cost:
                    frontier.modify(next_state, next_cost)
